<template>
  <div class="min-h-screen">
    <!-- 顶部导航 -->
    <nav class="bg-white/80 backdrop-blur-sm shadow-sm sticky top-0 z-50 transition-all duration-300">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
          <!-- 左侧 Logo 和标题 -->
          <div class="flex items-center gap-3">
            <div class="flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer">
              <div class="i-heroicons-sparkles text-2xl text-blue-600 animate-bounce"></div>
              <span class="text-xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">智能菜单助手</span>
            </div>
            <div class="hidden sm:flex items-center gap-6 ml-8">
              <a href="#" class="text-gray-600 hover:text-blue-600 transition-colors text-sm flex items-center gap-1">
          ...
          <div class="flex items-center gap-4">
            <button class="btn-hover px-4 py-2 text-sm text-gray-600 hover:text-blue-600 transition-colors flex items-center gap-1">
              <div class="i-heroicons-question-mark-circle"></div>
              <span class="hidden sm:inline">使用帮助</span>
            </button>
            <button class="btn-hover px-4 py-2 text-sm bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors flex items-center gap-1">
    ...
    <main class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent mb-2">智能菜单推荐</h1>
        <p class="text-gray-600">为您定制完美的用餐计划</p>
      </div>

      <!-- 表单卡片 -->
      <div class="bg-white/80 backdrop-blur-sm rounded-lg shadow p-6 mb-6 card-hover">
        <form @submit.prevent="handleSubmit">
          ...
          <div class="flex justify-end">
            ...
            >
              <div v-if="isLoading" class="i-heroicons-arrow-path animate-spin"></div>
      ...
      <Transition name="fade">
        <div v-if="menu" class="bg-white/80 backdrop-blur-sm rounded-lg shadow p-6 card-hover mb-6">
          <h2 class="text-xl font-bold bg-gradient-to-r from-green-600 to-teal-600 bg-clip-text text-transparent mb-4 flex items-center gap-2">
          ...
          <div class="space-y-4 mb-8">
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <span class="px-2 py-1 text-xs rounded-full" :class="getDishTypeClass(dish.type)">
                  {{ dish.type }}
                </span>
              </div>
              <button
              ...
              >
                <div :class="[isFavorite(dish) ? 'i-heroicons-star-solid text-yellow-400' : 'i-heroicons-star text-gray-400']"></div>
              </button>
            </div>
          </div>
          ...
          <div class="border-t border-gray-100 pt-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div class="p-4 bg-orange-50 rounded-lg">
                ...
                <div class="text-lg font-semibold text-gray-900">{{ menu.totalCalories || 0 }} 千卡</div>
              </div>
              <div class="p-4 bg-blue-50 rounded-lg">
                ...
                <div class="text-lg font-semibold text-gray-900">{{ menu.totalProtein || 0 }}g</div>
              </div>
              <div class="p-4 bg-yellow-50 rounded-lg">
                ...
                <div class="text-lg font-semibold text-gray-900">{{ menu.totalFat || 0 }}g</div>
              </div>
              <div class="p-4 bg-green-50 rounded-lg">
                ...
                <div class="text-lg font-semibold text-gray-900">{{ menu.totalCarbs || 0 }}g</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- 历史记录 -->
      <Transition name="fade">
        <div v-if="history.length > 0" class="bg-white/80 backdrop-blur-sm rounded-lg shadow p-6 card-hover">
            ...
            >
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded-full">
                  {{ item.menu.peopleCount }}人 · {{ getMealTypeName(item.menu.mealType) }}
                </span>
              </div>
